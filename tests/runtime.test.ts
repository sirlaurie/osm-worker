...
for (const schema of [1, 2] as const) {
  test(`Miniflare serves schema ${schema} data and preserves conditional uploads and leased publication`, async () => {
    ...
    try {
      ...
      try {
        assert.equal(
          (await fetch(`${service.worker}/__test/ready`)).status,
          204,
        );
        ...
        async function upload(
          ...
          const url = `${service.r2}/test-osm/${kind}/${hash}.${kind === "packs" ? "bin" : "json"}`;
          const signedHeaders = {
            Authorization:
        ...
        const bytes = Buffer.from(JSON.stringify([record, secondRecord]));
        const farBytes = Buffer.from(
          JSON.stringify([
            { ...record, id: "osm_node_3", lat: 0.05, lon: 0.05 },
          ]),
        );
        const hash = (bytes: Buffer) =>
          createHash("sha256").update(bytes).digest("hex");
        const block = hash(bytes);
        const dataHash =
          schema === 1
            ? await upload("blocks", [record, secondRecord])
            : await upload("packs", Buffer.concat([farBytes, bytes]));
        ...
        const manifest: Manifest =
          schema === 1
            ...
            : {
                ...
                schema: 2,
                cells: {
                  [cellFor(0, 0)]: [[block, 0, farBytes.length, bytes.length]],
                  [cellFor(0.05, 0.05)]: [
                    [hash(farBytes), 0, 0, farBytes.length],
                  ],
                },
                packs: [dataHash],
                count: 3,
        ...
        assert.deepEqual(await objects.json(), {
          keys: [
            schema === 1 ? `blocks/${dataHash}.json` : `packs/${dataHash}.bin`,
